...
class Request extends AbstractRequest implements RequestInterface
{
     ...
     * @param string $select
     */
    public function __construct(int $offset = 0, int $limit = 100, string $filter = '', string $select = '*')
    {
        $this->offset = $offset;
        $this->limit = $limit;
        $this->filter = $filter;
    ...
    {
        return sprintf('%s/salesInvoices', config('wtg.rest.admin_code'));
    }
