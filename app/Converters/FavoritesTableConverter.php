...
class FavoritesTableConverter extends AbstractTableConverter
{
    ...
    public function createModel(array $data): ?Model
    {
        try {
            ...
            Log::warning(
                "[Favorites table converter] Invalid data in favorites column for customer " . $data['username']
            );
        ...
        if (! $customer) {
            Log::warning(
                sprintf(
                    "[Favorites table converter] Skipped customer %s for company %s. Reason: Customer was not found in the database.",
                    $data['username'],
                    $data['company_id']
                )
            );
        ...
        foreach ($favorites as $favorite) {
            ...
            if (! $product) {
                Log::warning(
                    sprintf(
                        "[Favorites table converter] Removed product %s from customer favorites. Reason: Product was not found in the database.",
                        $favorite
                    )
                );
